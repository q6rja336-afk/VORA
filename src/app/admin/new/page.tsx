...
import { addContent } from '@/lib/supabase';
import { fetchTMDBMetadata } from '@/lib/tmdb';
import { useRouter } from 'next/navigation';
import { motion, AnimatePresence } from 'framer-motion';
import { ArrowRight, Save, Film, Tv, Type, AlignLeft, Star, Calendar, Globe, Clock, Layers, Image as ImageIcon, Video, Plus, ChevronDown, ChevronUp, Loader2, RefreshCw } from 'lucide-react';
...
export default function NewContentPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    language: 'العربية',
    seasons: 1,
    episodes: 0, // Total episodes (optional, can be sum of seasons)
    poster_url: '',
    backdrop_url: '',
    video_url: '', // For movies
    ...
    try {
      ...
      if (meta) {
        setFormData(prev => ({
          ...
          seasons_data: meta.type === 'series' ? meta.seasonsData : []
        }));
  ...
  // Update seasons_data when seasons count changes
  useEffect(() => {
    if (formData.type === 'series') {
      const count = formData.seasons || 1;
      setFormData(prev => {
        const newData = [...prev.seasons_data];
        if (newData.length < count) {
          for (let i = newData.length; i < count; i++) {
            newData.push({ seasonNumber: i + 1, episodes: [] });
          }
        } else if (newData.length > count) {
          newData.splice(count);
        }
        return { ...prev, seasons_data: newData };
      });
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await addContent(dataToSave);
      router.push('/admin');
    } catch (err: any) {
      console.error(err);
      alert('حدث خطأ أثناء الإضافة: ' + (err.message || 'خطأ غير معروف'));
      setLoading(false);
    }
  };
  ...
  return (
    <div style={{ maxWidth: '900px', margin: '0 auto', paddingBottom: '100px' }}>
      <div style={{ marginBottom: '40px' }}>
        ...
        <h1 style={{ fontSize: '32px', fontWeight: '800', color: '#fff' }}>إضافة محتوى جديد</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
        ...
        <FormSection title="المزامنة الذكية (TMDB)">
          ...
          <p style={{ fontSize: '13px', color: 'var(--outline)', marginTop: '-12px', opacity: 0.8 }}>
            * سيتم جلب كافة بيانات العمل تلقائياً بمجرد إدخال الكود.
          </p>
        </FormSection>

        <FormSection title={formData.type === 'movie' ? "روابط الفيلم" : "إدارة المواسم والحلقات"}>
          {formData.type === 'movie' ? (
...
const labelStyle: React.CSSProperties = {
  display: 'block', fontSize: '14px', fontWeight: '600', color: 'var(--on-surface-variant)', marginBottom: '10px'
};
...
const chipStyle: React.CSSProperties = {
  ...
  borderRadius: '8px', fontSize: '12px', fontWeight: '600', display: 'flex', alignItems: 'center'
};

const submitButtonStyle: React.CSSProperties = {
  padding: '20px', 
  background: 'linear-gradient(135deg, #4f61ff 0%, #3648e8 100%)', 
  color: '#fff',
  border: 'none', borderRadius: '16px', fontSize: '18px', fontWeight: '700',
  cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '12px',
  boxShadow: '0 10px 40px rgba(79, 97, 255, 0.35)',
  marginTop: '20px',
