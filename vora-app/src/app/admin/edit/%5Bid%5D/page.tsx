...
import { 
  ...
  Video, Plus, ChevronDown, ChevronUp, Loader2 
} from 'lucide-react';
    ...
    <div style={{ maxWidth: '900px', margin: '0 auto', paddingBottom: '100px' }}>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
        ...
        <FormSection title="المعلومات الأساسية">
          <FormInput label="العنوان" icon={<Type size={16} />} value={formData.title} onChange={(v: string) => setFormData({...formData, title: v})} placeholder="اسم الفيلم أو المسلسل..." required />
          ...
          <FormTextArea label="الوصف" icon={<AlignLeft size={16} />} value={formData.description} onChange={(v: string) => setFormData({...formData, description: v})} placeholder="اكتب وصفاً مختصراً باللغة العربية..." required />
        </FormSection>
        ...
        <FormSection title="الوسائط">
          <div style={grid2Style}>
            <FormInput label="رابط البوستر (Poster URL)" icon={<ImageIcon size={16} />} value={formData.poster_url} onChange={(v: string) => setFormData({...formData, poster_url: v})} required />
            <FormInput label="رابط الخلفية (Backdrop URL)" icon={<ImageIcon size={16} />} value={formData.backdrop_url} onChange={(v: string) => setFormData({...formData, backdrop_url: v})} required />
          </div>
        </FormSection>
        ...
        <FormSection title={formData.type === 'series' ? "إدارة المواسم والحلقات" : "روابط الفيلم"}>
          {formData.type === 'series' ? (
            ...
            <FormInput label="رابط الفيديو (Video URL)" icon={<Video size={16} />} value={formData.video_url} onChange={(v: string) => setFormData({...formData, video_url: v})} required />
          )}
        </FormSection>
